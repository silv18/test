...
import { StyleSheet,Switch, Dimensions,Text, View,TouchableOpacity,ActivityIndicator } from 'react-native';
...
export default function TableauScreen() {
  ...
  // }
  // record['fieldData']['RecordID'],record['fieldData']['date'],record['fieldData']['Fait']

  // const handleToggleMode = (recordID) => {
  ...
  const handleToggleMode1 = (recordID,dateRecord,Ok) => {
    ...
    .then(data => {
      ...
      // ligneEffacer.push(recordID);
      // console.log(ligneEffacer);
      // setIsEnabled((previousState) => !previousState);
    })
    .catch(error => {
...
return (
 <View style={styles.container}>
      ...
      {listRecord != undefined ? null : (
        
         listRecord.map((record) => {
          const faitValue = record['fieldData']['Fait'];
          //console.log(ligneEffacer.includes(record['fieldData']['RecordID']))
          const isDefaultChecked = faitValue === 1;
